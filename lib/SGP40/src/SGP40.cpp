...
SGP40::SGP40(TwoWire *pWire) : _pWire(pWire), _temperatureC(25)
{
    _deviceAddr = SGP40_ICC_ADDR;
    _relativeHumidity = 50.0;
    _temperatureC = 25.0;
}

bool SGP40::begin(uint32_t duration)
{
    _pWire->begin(10, 8, 100000);
...
uint8_t SGP40::checkCrc(uint8_t data1, uint8_t data2)
{
    uint8_t crc = 0xFF;
    uint8_t data[2];
    data[0] = data1;
...
void SGP40::setRhT(float relativeHumidity, float temperatureC)
{
...
void SGP40::write(uint8_t *cmd, uint8_t len)
{
...
uint16_t SGP40::getVoclndex(void)
{
    uint8_t data[3] = {0, 0, 0};
    int32_t value;
    int32_t vocIndex = 0;
